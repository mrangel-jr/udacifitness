...
export default class Live extends Component {
    ...
    componentDidMount () {
        ...
        .then(({ status }) => {
            if (status === 'granted') {
                // this.setState({coords: Math.floor((Math.random() * 360) + 1)})
    ...
    if (status === 'denied') {
      return (
        <View style={styles.center}>
            <Foundation name="alert" size={50} />
            <Text>
                Você negou nosso acesso e não podemos fazer nada.
                Vá nas Configurações e habilite a localização para o app.
            </Text>
        </View>
      );
    }
